...
-- TABLE: ranked_stats
-- ============================================================================
-- Stores ranked mode statistics per platform (PC, PlayStation, Xbox).
-- The `data` column holds the full RankedStats JSON structure.
...
create policy "Users can manage own ranked stats"
  on public.ranked_stats for all
  using (auth.uid() = user_id);

create index idx_ranked_stats_user_id on public.ranked_stats(user_id);
